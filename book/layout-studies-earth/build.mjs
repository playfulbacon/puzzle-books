...
// Text-width: the visible grid edges sit exactly on the text margins (72 px and 64 px), so the
// dots, the day line and the caption share the same left and right edges. Slitherlink's drawing
// pads half a cell around the dots, so the box is sized and offset to put the dots on the margins.
const hiTextWidth = (() => { const p = P.slither, cols = p.params.cols, visible = 672 - 72 - 64, cell = visible / cols, size = cell * (cols + 1), left = 72 - cell / 2, top = (960 - size) / 2; return wrap(`<div class="page">
  <div style="position: absolute; left: 72px; right: 64px; top: 96px; display: flex; justify-content: space-between; align-items: baseline;"><div style="display: flex; align-items: baseline; gap: 14px;">${dayInk(22)}${kanjiInk(22)}</div>${quietDots(2, 5, 5)}</div>
  ${inkedAt(p, size, left, top)}
  <div style="position: absolute; left: 72px; bottom: 64px;">${whisper(p, { size: 10, width: 536 })}</div>
  <div class="folio" style="position: absolute; bottom: 30px; right: 64px;">30</div>
</div>`); })();
...
writeFileSync(join(OUT, "DlSlitherlink.dc.html"), formatPageDotsLow(P.slither, 22, 2, 30));
writeFileSync(join(OUT, "DlMasyu.dc.html"), formatPageDotsLow(P.masyu, 63, 3, 71));
writeFileSync(join(OUT, "DlGokigen.dc.html"), formatPageDotsLow(P.gokigen, 47, 3, 55));
...
writeFileSync(join(OUT, "canvas.json"), JSON.stringify({
  artboards: [
    ...
    { ...A("DlSlitherlink.dc.html", 0, 0, "Slitherlink"), page: "page-9" },
    { ...A("DlMasyu.dc.html", 800, 0, "Masyu"), page: "page-9" },
    { ...A("DlGokigen.dc.html", 1600, 0, "Gokigen Naname"), page: "page-9" },

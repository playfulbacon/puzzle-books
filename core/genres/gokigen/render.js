...
import { INK, open, gridLines, cellTargets, DIGIT_FONT, esc } from "../../lib/svg.js";

/** cells: Int8Array (-1 none, 0 "\\", 1 "/") */
export function svg(puzzle, { cell = 40, cells = null, interactive = false, bad = false } = {}) {
  ...
  o += gridLines(rows, cols, cell, pad, { inner: cell * 0.018, outer: cell * 0.018 });
  ...
  for (let r = 0; r <= rows; r++) for (let c = 0; c <= cols; c++) {
    ...
    const x = pad + c * cell, y = pad + r * cell;
    o += `<circle cx="${x}" cy="${y}" r="${rad}" fill="#fff" stroke="${INK}" stroke-width="${cell * 0.03}"/>`;
    o += `<text x="${x}" y="${y + rad * 0.62}" text-anchor="middle" font-family="${DIGIT_FONT}" font-size="${rad * 1.6}" fill="${INK}">${esc(k)}</text>`;
  }
